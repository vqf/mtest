...
## Example 2 - two-tail binomial test
Only for a 2x2 contingency table, we can also test the hypothesis that one 
probability is higher than the other with the `tailed.m.test` function. In 
...
```{r tailed}
tailed.m.test(list(c(15, 11), c(41, 8)))
```

In a strict sense, this model does not contemplate the case where both 
probabilities are equal, and you may want to also discard that null 
hypothesis with `m.test`. However, using the same data for more than one 
test might be considered multiple hypothesis. You may want to adjust the 
limit p-value accordingly.
